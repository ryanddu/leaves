...
public final class CollHelper {
	...
	public static void main(String[] args) {
		Map<String, Set<String>> item1 = new HashMap<>();
		item1.put("a", CollUtil.newHashSet("1", "2"));
		item1.put("b", CollUtil.newHashSet("a"));
		Map<String, Set<String>> item2 = new HashMap<>();
		item2.put("a", CollUtil.newHashSet("3", "2"));
		item2.put("c", CollUtil.newHashSet("c"));

		Map<String, Set<String>> stringSetMap = putAll(item1, item2);
	 ...
	 * <p>
	 * guava 的 Maps.uniqueIndex方法可以实现： <br>
	 ...
	 * @return 唯一的map
	 */
	public static <K, V, M> Multimap<K, M> iterableToMultiMap(Iterable<V> values, Function<? super V, K> keyFunction,
			Function<? super V, M> valueFunction) {
		Iterator<V> iterator = values.iterator();
		checkNotNull(keyFunction);
		checkNotNull(valueFunction);
	...
	public static <T> List<String> split(Collection<T> list, Function<? super T, ?> function, CharSequence separator) {
		...
		return list.parallelStream()
			.map(function)
			.map(item -> StrUtil.splitToArray(String.valueOf(item), separator))
	...
	public static <T> List<String> split(Collection<String> list, CharSequence separator) {
		...
		return list.parallelStream()
			.map(item -> StrUtil.splitToArray(item, separator))
			.flatMap(Arrays::stream)
			.filter(ObjectUtil::isNotEmpty)
			.distinct()
			.collect(Collectors.toList());
