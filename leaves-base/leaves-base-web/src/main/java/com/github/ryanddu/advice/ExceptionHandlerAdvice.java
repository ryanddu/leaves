...
public class ExceptionHandlerAdvice implements EnvironmentAware {
    ...
    private boolean hasTrace = ClassUtils.isPresent("org.apache.skywalking.apm.toolkit.trace.TraceContext", ExceptionHandlerAdvice.class.getClassLoader());
    ...
    public R<Void> handleException(Exception e, HttpServletRequest request) {
		...
		if (this.showErrorDetail) {
			return this.wrapResult(R.failed(CommonCodeEnum.SERVER_ERROR_WITH_DETAIL.getCode()), new Object[]{ExceptionUtils.getStackTrace(e)});
		} else {
    ...
    public R<Void> MissingServletRequestParameter(MissingServletRequestParameterException ex, HttpServletRequest request) {
        this.logger.warn("缺少必要参数:[{}],[{}]", request.getRequestURI(), ex.getParameterName());
        return this.wrapResult(R.failed(CommonCodeEnum.REQUIRED_PARAMETER_MISSING.getCode()), new Object[]{ex.getParameterName()});
    }

    @ExceptionHandler({TypeMismatchException.class})
    public R<Void> requestTypeMismatch(TypeMismatchException ex, HttpServletRequest request) {
        this.logger.warn("参数类型不匹配:[{}],[{}],[{}]", new Object[]{request.getRequestURI(), ex.getPropertyName(), ex.getRequiredType()});
        return this.wrapResult(R.failed(CommonCodeEnum.PARAMETER_TYPE_DO_NOT_MATCH.getCode()), new Object[]{ex.getPropertyName()});
    }
    ...
    public R<Void> handleBindException(BindException ex) {
        ...
        return this.wrapResult(R.failed(CommonCodeEnum.PARAMETER_ERROR.getCode()),new Object[]{fieldError.getDefaultMessage()});
    }
    ...
    private R<Void> wrapResult(R r, Object[] args) {
        ...
        String message = this.messageSource.getMessage(String.valueOf(r.getCode()), args, LocaleContextHolder.getLocale());
        r.setMsg(message);
        return r;
    }
